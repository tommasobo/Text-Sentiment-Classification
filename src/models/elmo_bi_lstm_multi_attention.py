...
def run_model(args, input_obj):
    ...
    previous_out = layers.Dropout(0.25)(previous_out)
    previous_out = layers.Bidirectional(layers.LSTM(units=256, return_sequences=True, dropout = 0.2))(previous_out)
    previous_out = layers.Bidirectional(layers.LSTM(units=128, return_sequences=True, dropout = 0.2))(previous_out)
    previous_out = layers.Bidirectional(layers.LSTM(units=8, return_sequences=True, dropout = 0.2))(previous_out)
    previous_out = MultiHeadAttention(head_num=16, name='Multi-Head',)(previous_out)
    previous_out = layers.GlobalAveragePooling1D()(previous_out)
    previous_out = layers.Dense(32, activation='sigmoid')(previous_out)
    previous_out = layers.Dropout(0.35)(previous_out)
