...
def run_model(args, input_obj):
    ...
    previous_out = layers.Dropout(0.3)(previous_out)
    previous_out = layers.LSTM(units=500, return_sequences=True)(previous_out)
    previous_out = layers.LSTM(units=150)(previous_out)
    previous_out = layers.Dropout(0.3)(previous_out)
